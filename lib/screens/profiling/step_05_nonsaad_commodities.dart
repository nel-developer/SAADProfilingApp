...
class Step05NonSAADCommodities extends StatefulWidget {
  final VoidCallback onNext;
  final VoidCallback onBack;
  final VoidCallback? onHeaderBack;
  final ProfilingData? currentData;

  const Step05NonSAADCommodities({
    super.key,
  ...
  @override
  State<Step05NonSAADCommodities> createState() =>
      _Step05NonSAADCommoditiesState();
}
...
class _Step05NonSAADCommoditiesState extends State<Step05NonSAADCommodities> {
  ...
  void _loadCachedCommodities() async {
    try {
      ...
      debugPrint('📦 Step 5 loaded ${types.length} types from cache: $types');
      ...
      if (types.isEmpty) {
        ...
        if (bootstrapped) {
          ...
          debugPrint(
            '✅ Step 5 bootstrap loaded ${types.length} type(s): $types',
          );
        } else {
          debugPrint(
            '❌ Step 5 bootstrap failed or Firestore has no commodities.',
          );
        }
      }
      ...
      if (mounted) {
        ...
        // After cache is ready, restore any saved entries from currentData
  ...
  void _restoreSavedEntries() {
    ...
    final saved = widget.currentData!.nonSAADCommodities ?? [];
    if (saved.isEmpty) {
      _selectedType = widget.currentData!.nonSAADCommodityType;
      return;
    }
  ...
  void _autoSaveToCurrentData() {
    if (widget.currentData != null) {
      widget.currentData!.nonSAADCommodityType = _selectedType;
      ...
      widget.currentData!.nonSAADCommodities = allEntries;
    }
  }
  ...
  void _deleteEntry(int index) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('🗑️ Commodity removed'),
  ...
  @override
  void didUpdateWidget(Step05NonSAADCommodities oldWidget) {
  ...
  void _handleNext() {
    ...
    if (_selectedType != null) {
      ...
      allEntries.add({
        ...
        'saleMeth': _selectedSaleMeth,
        'productForm': _selectedProductForm,
        'pricingBasis': _selectedPricingBasis,
        'unit': resolvedCurrentUnit,
        if (_maleRequired) 'maleCount': _maleCountCtrl.text,
        if (_femaleRequired) 'femaleCount': _femaleCountCtrl.text,
        'totalWeight': _weightCtrl.text,
        'totalAmount': _amountCtrl.text,
  ...
  Widget build(BuildContext context) {
    ...
    return ProfilingStepWrapper(
      currentStep: 5,
      sectionTitle: 'Non-SAAD Commodity Type',
      onNext: _handleNext,
      onBack: _handleBack,
      ...
      child: Column(
        ...
        children: [
          ...
          if (_selectedProductForm != null) ...[
            ...
            // Add Another Commodity Button — create full dynamic form
            SizedBox(height: fieldGap),
            GestureDetector(
              onTap: () {
                setState(() {
                  _additionalForms.add(_CommodityForm());
                });
              },
              child: Container(
            ...
            // Display additional dynamic forms
            if (_additionalForms.isNotEmpty) ...[
              ...
              ListView.builder(
                ...
                itemBuilder: (context, formIdx) {
                  final form = _additionalForms[formIdx];
                  return Container(
                    margin: EdgeInsets.only(bottom: fieldGap),
                    padding: EdgeInsets.all(fieldGap * 0.5),
                    decoration: BoxDecoration(
                      color: DAColors.lightGrey.withOpacity(0.3),
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(
                        color: DAColors.primaryGreen,
                        width: 1,
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // Type and Commodity side-by-side
                        Row(
                          children: [
                            Expanded(
                              child: Column(
                                    ...
                                    child: _shadowedDropdown(
                                      ...
                                      onChanged: (val) => setState(() {
                                        ...
                                        form.commodityOptions = val != null
                                            ? _cache.getCommoditiesForType(val)
                                            : [];
                                        form.commodity = null;
                                        form.saleMeth = null;
                            ...
                            SizedBox(width: fieldGap * 0.5),
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  _label('Commodity', labelSize - 2),
                                  SizedBox(height: 4),
                                  SizedBox(
                                    height: fieldHeight - 6,
                                    child: _shadowedDropdown(
                            ...
                            IconButton(
                              ...
                              onPressed: () => setState(() {
                                _additionalForms[formIdx].dispose();
                                _additionalForms.removeAt(formIdx);
                              }),
                            ),
                          ],
                        ),
                              ...
                              if (form.saleMethOptions.isNotEmpty)
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      _label('Sale Method', labelSize - 3),
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment:
                                        CrossAxisAlignment.start,
                                    children: [
                                      _label('Product Form', labelSize - 3),
                                      const SizedBox(height: 4),
                                      SizedBox(
                                        height: fieldHeight - 6,
                                        child: _shadowedDropdown(
                                ...
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment:
                                        CrossAxisAlignment.start,
                                    children: [
                                      _label('Male', labelSize - 3),
                                      SizedBox(height: 2),
                                      _textField(
                                        hintText: 'Males',
                                        onChanged: (val) {},
                                        fieldHeight: fieldHeight - 8,
